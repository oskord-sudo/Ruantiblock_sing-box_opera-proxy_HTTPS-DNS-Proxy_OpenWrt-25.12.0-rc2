...
# ОБНОВЛЕНИЕ ИНДЕКСОВ ПАКЕТОВ перед установкой
echo "Updating package indexes..."
apk update

# Установка необходимых пакетов
apk add unzip sing-box

echo "Install opera-proxy client"
service stop vpn > /dev/null
rm -f /usr/bin/vpns /etc/init.d/vpn

url="https://github.com/NitroOxid/openwrt-opera-proxy-bin/releases/download/1.13.1/opera-proxy-1.13.1-r1.apk"
destination_file="/tmp/opera-proxy.apk"
